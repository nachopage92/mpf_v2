function [rhs,ind_bcD] = Beam_RHS_lme(x_nodes,options,parameters)
% function [rhs,ind_Dirichlet] = Beam_RHS_lme(x_nodes,options,parameters);
%
...
rhs_x = zeros(length(y1D),1);
rhs_y = zeros(length(y1D),1);
