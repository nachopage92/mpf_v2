function [D, dD] = Rfunction_equiv(mesh,v,p,spow,mpow)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Purpose
% =======
...
%   D(j)    = exp(-w^(-spow));
%   dD(j,:) = exp(-w^(-spow))*spow*w^(-spow-1)*dw;
...
%Fast version of Marino's implementation
N    = length(rho);
ids  = 1:N;
ids  = ids(rho<eps);
nrho0= length(ids);
...
if nrh0>2
  ...
  hold off
  axis equal
  fprintf(1,'Rfunction_equiv: rho have %d zero values at x=[%4.2f %4.2f]\n',nrh0,x(1),x(2));
