program Finite_Point_Method_Solver
    ...
    INTEGER,PARAMETER :: MAXCLD = 50  ! numero maximo permitido por nubes
    ...
    REAL*8,ALLOCATABLE :: RM_FWLS(:,:,:),RM_MaxEnt(:,:,:),RM_MaxEnt_simple(:,:,:)
...
!--------------> ASIGNAR MEMORIA
    ...
    ALLOCATE( S(MAXCLD,NNOD),S1(MAXCLD,NNOD) )
    ALLOCATE( F(NNOD),PRESS(NNOD),DU(NNOD),DV(NNOD),U(NNOD),V(NNOD) )
...
!--------------> IMPRESION DE LOS DESPLAZAMIENTOS
    WRITE(1,'(A15,I4,I6,3I4)') 'U_APPROX'
    
    DO INOD=1,NNOD
    
       CALL EVAVECT(INOD,USOL,aux,1)
       WRITE(1,'(I6,E25.13)') INOD,USOL
    END DO

!--------------> IMPRESION DE LOS ESFUERZOS
      
    WRITE(1,'(A15,I4,I6,3I4)') 'DU_APPROX'
    
    DO INOD=1,NNOD
        
        CALL EVAVECT(INOD,DUDX,aux,2)
        CALL EVAVECT(INOD,DUDY,aux,3)
        
        WRITE(1,'(I6,2E25.13)') INOD,DUDX,DUDY
...
!--------------> ENSAMBLA LAS ECUACIONES EN EL DOMINIO
    ...
    DO INOD=1,NNOD
      ...
      DO J=1,NPUNTOS(INOD)
         S(J,INOD) = ! insertar ecuacion dominio
      END DO
...
!cccc-----> ec.contorno-h/2*(ec.dominio)=0
            ...
            S(J,INOD) = ! insertar ecuacion aqui, ¿como ensamblar termino estabilizador?
...
!----------------------------------------------------------!
    ...
    subroutine N_DN_D2N
        ...
        ! despejar espacio
        !deallocate(RM_FWLS,RM_MaxEnt,RM_MaxEnd_simple)
    ...
    SUBROUTINE N_DN_D2N_MaxEnt_simple
        use lme_simplificado , only : maxent2d_shapefcn_v2

        integer inod,np,maxiter,j
        real*8 beta_,gamma_,tol,gamma(nnod),hcar,init_lam(2),xstar(2)
        real*8,allocatable :: phi(:),dphi(:,:),ddphi(:,:,:),xyz(:,:),beta(:)
        ...
        ALLOCATE( RM_MaxEnt_simple(NCONS,MAXCLD,NNOD) ) ; RM_MaxEnt_simple = 0.d0
        ! NOTA : DMAX calculado anteriormente en N_DN_D2N

        do inod = 1,nnod ! recorre cada nodo estrella del dominio 

            ! numero de puntos por nube
            np = npuntos(inod)
        
            ! asignar memoria
            allocate(phi(np),dphi(2,np),ddphi(2,2,np),xyz(2,np),beta(np))

            call h_caract(inod,hcar) ! distancia internodal caracteristica

            do j = 1,np ! parametro de localidad
                beta(j) = gamma(CL_CONECT(j,inod))/hcar**2.0d0
            end do 

            ! declarar variables de entrada
            xyz(1,1:np) = X(CL_CONECT(1:np,inod))
            xyz(2,1:np) = Y(CL_CONECT(1:np,inod))
            xstar(1:2) = (/ X(CL_CONECT(1,inod)) , Y(CL_CONECT(1,inod)) /)
            ...
            if ( any( inod .eq. inorm_node ) ) then ! is boundary?
                ...
                call maxent2d_shapefcn_v2( np, xyz, xstar, beta, maxiter,&
                                & init_lam, tol, phi, dphi, ddphi )
            end if

            RM_MaxEnt_simple(1,1:np,inod) = phi(1:np)
            RM_MaxEnt_simple(2,1:np,inod) = dphi(1,1:np)
            RM_MaxEnt_simple(3,1:np,inod) = dphi(2,1:np)
            RM_MaxEnt_simple(4,1:np,inod) = ddphi(1,1,1:np)
            RM_MaxEnt_simple(5,1:np,inod) = ddphi(2,1,1:np) ! o ddphi(1,2,1:np)
            RM_MaxEnt_simple(6,1:np,inod) = ddphi(2,2,1:np)


            ! despejar memoria
            deallocate(phi,dphi,ddphi,xyz,beta)

        end do

        return
    END SUBROUTINE N_DN_D2N_MaxEnt_simple
